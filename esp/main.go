...
func renderEntityInfo(hdc win.HDC, tPen uintptr, gPen uintptr, oPen uintptr, hPen uintptr, rect Rectangle, hp int32, name string, headPos Vector3) {
	...
	if nameRendering {
		...
		if err != nil {
			fmt.Println("Error converting string to UTF16:", err)
			return
		}
		...
		x := int32(rect.Left) + (int32(rect.Right) - int32(rect.Left))/2
		y := int32(rect.Top) - 14
	
		win.TextOut(hdc, x, y, text, int32(len(name)))
	}
...
func initWindow(screenWidth uintptr, screenHeight uintptr) win.HWND {
	...
	className, err := windows.UTF16PtrFromString("csinfWindow")
	if err != nil {
		logAndSleep("Error creating window class name", err)
		return 0
	}
	windowTitle, err := windows.UTF16PtrFromString("csinf - " + version)
	if err != nil {
...
func cliMenu() {
	for {
		if dev {
			fmt.Print(chalk.Green.Color("csinf - " + version + " - DEV\n\n"))
		} else {
			fmt.Print(chalk.Green.Color("csinf - " + version + "\n\n"))
		}

		// Print menu options
		printOption("[1] ESP", esp)
		printOption("[2] Team check", teamCheck)
		printOption("[3] Head circle", headCircle)
		printOption("[4] Skeleton rendering", skeletonRendering)
		printOption("[5] Box rendering", boxRendering)
		printOption("[6] Health bar rendering", healthBarRendering)
		printOption("[7] Health text rendering", healthTextRendering)
		printOption("[8] Name rendering", nameRendering)
		fmt.Println(chalk.Red.Color("[9] Adjust frame delay [" + fmt.Sprint(frameDelay) + "]"))
		fmt.Println(chalk.Red.Color("[10] Exit"))
		if dev {
			printOption("[69420] Test (DEV)", test)
		}
...
func printOption(label string, isEnabled bool) {
	if isEnabled {
		fmt.Println(chalk.Green.Color(label + " [ON]"))
	} else {
		fmt.Println(chalk.Red.Color(label + " [OFF]"))
	}
}
